...
def align_index_files(
    ...
    aligner = pavcall.params.PavParams(wildcards['asm_name'], PAV_CONFIG, ASM_TABLE).aligner
...
rule align_all:
    input:
        align_trim_none=lambda wildcards: pavcall.pipeline.expand_pattern(
...
rule align_depth:
    ...
    run:
        pavcall.align.tables.align_depth_table(
            ...
            df_fai=agglovar.fa.read_fai(pavcall.pipeline.expand_fofn(input.ref_fofn)[1]),
...
rule align_tables:
    ...
    run:
        ...
        lc_model = pavcall.align.lcmodel.get_model(pav_params.lc_model)

        # Trim-none: Read alignments as a BED file.
        df_none = pavcall.align.tables.sam_to_align_table(
        ...
        if pav_params.debug:
            try:
                pavcall.align.records.check_matched_bases(df_none, ref_fa_filename, qry_fa_filename)
        ...
        # Trim-qry
        df_qry = pavcall.align.trim.trim_alignments_qry(
            df=df_none,
            df_qry_fai=df_qry_fai,
            score_model=score_model,
        )
        ...
        try:
            for row in df_qry.iter_rows(named=True):
                pavcall.align.records.check_record(row, df_qry_fai)
        except Exception as e:
            raise ValueError(f'Failed to check alignment records (qry trimmed): {wildcards.asm_name}-{wildcards.hap}: {e}') from e

        if pav_params.debug:
            try:
                pavcall.align.records.check_matched_bases(df_qry, ref_fa_filename, qry_fa_filename)
            except Exception as e:
        ...
        # Trim-ref
        df_qryref = pavcall.align.trim.trim_alignments_ref(
            df=df_qry,
            df_qry_fai=df_qry_fai,
            score_model=score_model,
        ...
        if pav_params.debug:
            try:
                pavcall.align.records.check_matched_bases(df_qryref, ref_fa_filename, qry_fa_filename)
            except Exception as e:
...
rule align_map:
    ...
    run:
        ...
        ref_fa = str(pavcall.pipeline.expand_fofn(input.ref_fofn)[0])
        qry_fa = str(pavcall.pipeline.expand_fofn(input.qry_fofn)[0])
...
def _align_export_all(wildcards):
    ...
    return pavcall.pipeline.expand_pattern(
...
rule align_export:
    ...
    run:
        ...
        SAM_TAG = fr'@PG\tID:PAV-{wildcards.trim}\tPN:PAV\tVN:{pavcall.__version__}\tDS:PAV Alignment trimming {pavcall.align.trim.TRIM_DESC[wildcards.trim]}'
