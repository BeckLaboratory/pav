...
rule call_tables_hap:
    input:
        pq_inter=lambda wildcards: pavcall.pipeline.expand_pattern(
...
rule call_integrate_sources:
    ...
    output:
        ...
        inter_segment='results/{asm_name}/call_hap/inter_segment_{hap}.parquet',
        inter_ref_trace='results/{asm_name}/call_hap/inter_reftrace_cpx_{hap}.parquet',
    ...
    run:
        ...
        df_trim = pavcall.call.integrate.read_trim_table(
            df_align_none,
            df_align_qry,
            df_align_qryref,
        ).lazy()
        ...
        df_discord_schema = {
            key: val for key, val in pavcall.schema.VARIANT.items() if key in {'chrom', 'pos', 'end', 'id'}
        }
        ...
        df_inner_schema = {
            'align_index': pavcall.schema.ALIGN['align_index'],
            'id': pavcall.schema.VARIANT['id'],
        }
        ...
        df_var_index_id_schema = {
            'var_index': pavcall.schema.VARIANT['var_index'],
            'id': pavcall.schema.VARIANT['id'],
        }
        ...
        for sourcetype_vartype in param_dict.keys():
            ...
            if not (is_lg or is_inv):
                df = pavcall.call.integrate.apply_trim_filter(df, df_trim)

            # Filter DISCORD
            if filter_discord or filter_inner:
                df = pavcall.call.integrate.apply_discord_and_inner_filter(
                    df,
            ...
            df = pavcall.call.integrate.id_and_version(
                df=df,
                is_snv=is_snv,
            ...
            if is_cpx:
                pavcall.call.integrate.add_cpx_derived(
                    df=df,
            ...
            if do_write:
                ...
                df = (
                    ...
                    .select([col for col in pavcall.schema.VARIANT.keys() if col in df.collect_schema().names()])
                )
...
rule call_inter:
    ...
    run:
        ...
        min_anchor_score = pavcall.lgsv.chain.get_min_anchor_score(pav_params.min_anchor_score, score_model)
        ...
        with open(log.log, 'w') as log_file:
            ...
            caller_resources = pavcall.lgsv.resources.CallerResources(
            ...
            with tempfile.TemporaryDirectory(
                ...
                lgsv_list = pavcall.lgsv.call.call_from_align(
            ...
            schema_insdel = {
                col: pavcall.schema.VARIANT[col] for col in
                    pavcall.schema.VARIANT.keys() if col in (
                        pavcall.lgsv.variant.InsertionVariant.row_set({'var_index'}) |
                        pavcall.lgsv.variant.DeletionVariant.row_set({'var_index'})
                )
            }
            ...
            schema_inv = {
                col: pavcall.schema.VARIANT[col] for col in
                    pavcall.schema.VARIANT.keys() if col in (
                        pavcall.lgsv.variant.InversionVariant.row_set({'var_index'})
                )
            }
            ...
            schema_cpx = {
                col: pavcall.schema.VARIANT[col] for col in
                    pavcall.schema.VARIANT.keys() if col in (
                        pavcall.lgsv.variant.ComplexVariant.row_set({'var_index'})
                )
            }

            schema_ref_trace = (
                    pavcall.lgsv.struct.REF_TRACE_SCHEMA |
                    {'var_index': pavcall.schema.VARIANT['var_index']}
            )

            schema_segment = (
                    pavcall.lgsv.interval.SEGMENT_TABLE_SCHEMA |
                    {'var_index': pavcall.schema.VARIANT['var_index']}
            )
...
rule call_intra_inv:
    input:
        align_none='results/{asm_name}/align/{hap}/align_trim-none.parquet',
        pq_flag='results/{asm_name}/call_hap/intra_inv_flagged_sites_{hap}.parquet',
        ref_fofn='data/ref/ref.fofn',
        qry_fofn='data/query/{asm_name}/query_{hap}.fofn',
    output:
    ...
    run:
        ...
        # Call per query
        df_inv = pavcall.call.intra.variant_tables_inv(
            df_align=df_align,
            df_flag=df_flag,
            ref_fa_filename=str(ref_fa_filename),
            qry_fa_filename=str(qry_fa_filename),
...
rule call_intra_inv_flag:
    ...
    run:
        ...
        df_align = pl.scan_parquet(input.align_none)
        ref_fai_filename = pavcall.pipeline.expand_fofn(input.ref_fofn)[1]
        qry_fai_filename = pavcall.pipeline.expand_fofn(input.qry_fofn)[1]
        ...
        # Call per query
        (
            pavcall.call.intra.variant_flag_inv(
                df_align=df_align,
                df_snv=df_snv,
                df_insdel=df_insdel,
                df_ref_fai=df_ref_fai,
...
rule call_intra_snv_insdel:
    ...
    run:
        ...
        with tempfile.TemporaryDirectory(
            ...
            df_snv, df_insdel = pavcall.call.intra.variant_tables_snv_insdel(
                df_align=df_align,
                ref_fa_filename=str(ref_fa_filename),
                qry_fa_filename=str(qry_fa_filename),
