...
import pavcall.align
import pavcall.params
import pavcall.const
import pavcall.io
import pavcall.kde
import pavcall.region
import pavcall.seq
...
def try_intra_region(
    ...
    :param kde_model: Kernel density estimator function. Expected to be a `pavcall.kde.KdeTruncNorm` object, but can
    ...
    """
    if log_file is None:
        log_file = pavcall.io.NullWriter()
    ...
    if pav_params is None:
        pav_params = pavcall.params.PavParams()
    ...
    while inv_iterations <= min_expand and inv_iterations <= max_expand:
        ...
        df_rl = pavcall.kde.rl_encoder(df_kde)
...
def get_inv_row(
        region_ref_inner: Optional[pavcall.region.Region] = None,
        region_ref_outer: Optional[pavcall.region.Region] = None,
        region_qry_inner: Optional[pavcall.region.Region] = None,
        region_qry_outer: Optional[pavcall.region.Region] = None,
    ...
    if all(none_vals):
        region_ref_inner = pavcall.region.Region('NA', 0, 0)
        region_ref_outer = pavcall.region.Region('NA', 0, 0)
        region_qry_inner = pavcall.region.Region('NA', 0, 0)
        region_qry_outer = pavcall.region.Region('NA', 0, 0)
        align_index = -1
...
def get_state_table(
        region_ref: pavcall.region.Region,
        region_qry: pavcall.region.Region,
        ref_fa_filename: str,
        qry_fa_filename: str,
        df_ref_fai: pl.DataFrame,
        df_qry_fai: pl.DataFrame,
        ...
        kde_model: Optional[pavcall.kde.Kde] = None,
        max_ref_kmer_count: int = pavcall.const.INV_MAX_REF_KMER_COUNT,
    ...
    """
    if k_util is None:
        k_util = agglovar.kmer.util.KmerUtil(pavcall.const.INV_K_SIZE)
    ...
    ref_kmer_count = pavcall.seq.ref_kmers(region_ref_exp, ref_fa_filename, k_util)
    ...
    seq_qry = pavcall.seq.region_seq_fasta(region_qry_exp, qry_fa_filename, rev_compl=False)
...
def cluster_table(
        ...
        pav_params: Optional[pavcall.params.PavParams]
) -> pl.DataFrame:
...
def _resolve_inv_from_rl(
        ...
        min_inv_kmer_run: int = pavcall.const.INV_MIN_INV_KMER_RUN,
...
def _walk_to_regions(
        ...
        region_qry: pavcall.region.Region,
        k_size: int = 0
) -> tuple[pavcall.region.Region, pavcall.region.Region]:
    ...
    region_qry_outer = pavcall.region.Region(
        region_qry.chrom,
    ...
    region_qry_inner = pavcall.region.Region(
        region_qry.chrom,
...
def _expand_region(
    ...
    """
    expand_bp = int(len(region_ref) * pavcall.const.INV_EXPAND_FACTOR)
