...
class Variant(object, metaclass=abc.ABCMeta):
    ...
    def __getattr__(self, name):
        ...
        if name == 'is_rev':
            ...
            return self.interval.region_ref if not self.is_null() else region.Region('*', 0, 1)

        elif name == 'QRY_REGION':
            return self.interval.region_qry if not self.is_null() else region.Region('*', 0, 1)
...
class InsertionVariant(Variant):
    ...
    def __init__(self, interval, caller_resources, var_region_kde=None):
        ...
        if (dup_row := interval.df_segment.iloc[1])['IS_ALIGNED']:
            self.templ_region = region.Region(dup_row['#CHROM'], dup_row['POS'], dup_row['END'])
...
class TandemDuplicationVariant(InsertionVariant):
    ...
    def __init__(self, interval, caller_resources, var_region_kde=None):
        ...
        if (svlen := qry_end - qry_pos) > 0:
            ...
            self.region_qry = region.Region(interval.region_qry.chrom, qry_pos, qry_end)
    ...
    def __init__(self, interval, caller_resources, var_region_kde=None):
        ...
        if df_int_aligned.shape[0] == 1 and all(df_int_aligned['IS_REV'] != interval.is_rev) and all(df_int_aligned['#CHROM'] == interval.chrom):
            ...
            if (
                ...
                self.region_ref_outer = region.Region(interval.chrom, ref_pos_outer, ref_end_outer)
                self.region_ref_inner = region.Region(interval.chrom, ref_pos_inner, ref_end_inner)

                self.region_qry_outer = region.Region(interval.qry_id, qry_pos_outer, qry_end_outer)
                self.region_qry_inner = region.Region(interval.qry_id, qry_pos_inner, qry_end_inner)
