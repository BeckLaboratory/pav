...
class LCAlignModel(abc.ABC):
    ...
    Attributes:
        lc_model_def: LC model definition dictionary from a model definition JSON file. If None, the child class
            must create one or let this parent class raise an exception. Null models do this, most others will not.
        model_dir: Directory containing the model definition JSON file. If `None`, no related resources can be loaded
            and the full model definition must be in `lc_model_def`.
    """
    lc_model_def: Optional[dict[str, Any]] = field(repr=False)
    model_dir: Optional[pathlib.Path | importlib.resources.abc.Traversable] = field(repr=False)
    ...
    def __post_init__(self):
        ...
        if lc_model_def.get('type', None) is None:
            raise ValueError(f'LC align model definition is missing the "type" attribute')

        lc_model_def['type'] = str(lc_model_def['type']).strip()

        if lc_model_def['type'].strip() == '':
            raise ValueError(f'LC align model definition has an empty "type" attribute')

        lc_model_def['type_version'] = int(lc_model_def.get('type_version', 0))

        lc_model_def['score_model'] = score.get_score_model(self.lc_model_def.get('score_model', None))

        lc_model_def['features'] = features.FeatureGenerator.get_feature_list(lc_model_def.get('features', []))

        lc_model_def['score_prop_conf'] = float(
            self.lc_model_def.get('score_prop_conf', features.ALIGN_FEATURE_SCORE_PROP_CONF)
        )
        ...
        lc_model_def['allow_unknown_attributes'] = util.as_bool(
    ...
    @property
    def score_model(self) -> score.ScoreModel:
    ...
    def resource_path(
        ...
        Args:
            resource_name: Name of the resource relative to the model directory.

        Returns:
            Path to the resource.

        Raises:
            ValueError: If the model directory was not specified when the model class was instantiated.
        """

        if self.model_dir is None:
            raise ValueError(f'Cannot retrieve resource {resource_name}: No model directory specified')

        return self.model_dir / resource_name

    def get_feature_table(
            self,
            df: pl.DataFrame,
            existing_score_model: Optional[str | score.ScoreModel | bool] = None,
        ...
        Args:
            df: DataFrame or Series of alignment records. If a Series is given, then it is converted to a single-row
                DataFrame.
            existing_score_model: The score model used to compute features already in `df`. If this is not None and
            ...
            df_qry_fai: FAI file for query sequences (optional). Needed if features require the query length (i.e.
                proportion of a query in an alignment record).
            only_features: If True, only features are returned. If False, the full alignment table with features is
                returned.
        ...
        force_score = self.score_model != score.get_score_model(existing_score_model)

        try:
            return features.FeatureGenerator(
