...
from . import util as pavcall_call_util
...
#
# class InvCall:
#     region_ref: pavcall_region.Region
#     region_qry: pavcall_region.Region
#     is_rev: bool
#     svlen: int
#     id: str
#     score: float
#     region_ref_outer: pavcall_region.Region
#     region_qry_outer: pavcall_region.Region
#
#     """
...
#
#     region_ref = pavcall_region.Region(row['#CHROM'], row['POS'], row['END'])
#     region_qry = pavcall_region.Region(row['QRY_ID'], row['QRY_POS'], row['QRY_END'], is_rev=row['QRY_STRAND'] == '-')
#     # region_qry = pavcall_region.region_from_string(row['QRY_REGION'], is_rev=row['QRY_STRAND'] == '-')
#
#     return InvCall(
...
#
#         region_ref_outer=pavcall_region.region_from_string(row['REGION_REF_OUTER']) if 'REGION_REF_OUTER' in row else None,
#         region_qry_outer=pavcall_region.region_from_string(row['REGION_QRY_OUTER'], is_rev=region_qry.is_rev) if 'REGION_QRY_OUTER' in row else None
#     )


def scan_for_inv(
        region_flag: pavcall_region.Region,
        ref_fa_filename: str | tuple[str, str],
        qry_fa_filename: str | tuple[str, str],
        align_lift: pavcall_align.lift.AlignLift,
        pav_params: Optional[pavcall_params.PavParams] = None,
        k_util: agglovar.kmer.util.KmerUtil = None,
        kde_model: pavcall_kde.Kde=None,
    ...
    Args:
        region_flag: Flagged region to begin scanning for an inversion.
        ref_fa_filename: Reference FASTA filename. May be a string (expects "<filename>.fai" to exist) or a tuple of
             the FASTA and FAI file names, respectively.
        qry_fa_filename: Query FASTA filename. May be a string (expects "<filename>.fai" to exist) or a tuple of
             the FASTA and FAI file names, respectively.
        align_lift: Alignment lift-over tool (pav.align.AlignLift).
        pav_params: PAV parameters.
        k_util: K-mer utility. If `None`, a k-mer utility is created with PAV's default k-mer size
            (`pavcall.const.K_SIZE').
        kde_model: Kernel density estimator function. Expected to be a `pavcall.kde.KdeTruncNorm` object, but can
            be any object with a similar signature. If `None`, a default `kde` estimator is used.
        match_index: If `True`, then both left and right regions must land on the same alignment record. Set to
        ...
        stop_on_lift_fail: If `True`, stop if the reference-to-query lift fails for any reason (i.e. breakpoints
        ...
        log_file: Log file (open file handle) or `None` for no log.

    Returns:
        An `InvCall` object describing the inversion found or `None` if no inversion was found.
    ...
    if log_file is None:
        log_file = pavcall_io.NullWriter()

    # Init
    log_file.write(f'Intra-align inversion region {region_flag}: Scanning for inversions\n')
    log_file.flush()

    # Get FASTA/FAI files
    if isinstance(ref_fa_filename, tuple):
        if len(ref_fa_filename) != 2:
            raise ValueError(f'If ref_fa_filename is a tuple, it must contain two elements: Found ({len(ref_fa_filename)})')

        ref_fa_filename, ref_fai_filename = ref_fa_filename

    else:
        ref_fai_filename = ref_fa_filename + '.fai'

    if isinstance(qry_fa_filename, tuple):
        if len(qry_fa_filename) != 2:
            raise ValueError(f'If qry_fa_filename is a tuple, it must contain two elements: Found ({len(qry_fa_filename)})')

        qry_fa_filename, qry_fai_filename = qry_fa_filename

    else:
        qry_fai_filename = qry_fa_filename + '.fai'
    ...
    while inv_iterations <= min_expand and inv_iterations <= max_expand:
        ...
        if inv_iterations > 0:
            ...
            region_ref = expand_region(region_ref, df_rl, ref_fai)

            if len(region_ref) == last_len:
                # Stop if expansion had no effect
                log_file.write(f'Intra-align inversion region {region_flag}: Reached reference limits\n')
                log_file.flush()
        ...
        if region_limit is not None and 0 < region_limit < len(region_ref):
            log_file.write(f'Intra-align inversion region {region_flag}: Region size exceeds max: {region_ref} ({len(region_ref):,d} > {region_limit:,d})\n')
            log_file.flush()

            return None

        # Get query region
        region_qry = lift_ref_to_qry(region_ref, align_lift, match_index=match_index)

        # Check for expansion through a break in the query sequence
        if region_qry is None:

            if stop_on_lift_fail:
                log_file.write(f'Intra-align inversion region {region_flag}: Could not lift reference region onto query: {region_ref}\n')
                log_file.flush()

                return None

            log_file.write(f'Intra-align inversion region {region_flag}: Lift failed: {region_ref}=n')
            log_file.flush()
        ...
        if nc_qry is not None and region_qry.chrom in nc_qry.keys():
            ...
            if len(nc_tree_qry[region_qry.pos:region_ref.end]) > 0:
                log_file.write(f'Intra-align inversion region {region_flag}: Region overlaps no-call bases (query): {region_qry}')
                log_file.flush()

                return None

        # Log search
        log_file.write(f'Intra-align inversion region {region_flag}: Found region: {region_ref}\n')
        log_file.flush()

        # Create k-mer table
        df_kde = get_state_table(
            region_ref=region_ref, region_qry=region_qry,
            ref_fa_name=ref_fa_filename, qry_fa_name=qry_fa_filename,
        ...
        if df_kde is None:
            log_file.write(f'Intra-align inversion region {region_flag}: No informative reference k-mers in forward or reverse orientation in region\n')
            log_file.flush()

            return None

        if df_kde.shape[0] < min_kmers:
            log_file.write(f'Intra-align inversion region {region_flag}: Not enough k-mers in region to call: {df_kde.shape[0]} < {min_kmers}\n')
            log_file.flush()

            continue

        # Get run-length encoded states (list of (state, count) tuples).
        df_rl = pavcall_kde.rl_encoder(df_kde)
    ...
    if df_rl.iloc[0]['STATE'] != 0 or df_rl.iloc[-1]['STATE'] != 0:
        raise ValueError(f'Found INV region not flanked by reference sequence (program bug): {region_ref}')
    ...
    if inv_walk is None:
        log_file.write(f'Intra-align inversion region {region_flag}: Failed to resolve inversion breakpoints: {region_ref}: No walk along KDE states was found\n')
        log_file.flush()
    ...
    if region_ref_inner is None:
        log_file.write(f'Intra-align inversion region {region_flag}: Failed lifting inner INV region to reference: {region_qry_inner}\n')
        log_file.flush()
    ...
    if region_ref_outer is None or not region_ref_outer.contains(region_ref_inner):
        ...
        log_file.write(f'Intra-align inversion region {region_flag}: Failed lifting outer INV region to reference: {region_qry_outer}: Using inner coordinates\n')
        log_file.flush()
    ...
    if len(region_ref_inner) < len(region_qry_inner) * min_qry_ref_prop:
        log_file.write(
            (
                'Intra-align inversion region {}: Reference region too short: '
                'Reference region length ({:,d}) is not within {:.2f}% of the query region length ({:,d})\n'
            ).format(
                region_flag,
    ...
    if len(region_qry_outer) < len(region_ref_outer) * min_qry_ref_prop:
        log_file.write(
            (
                'Intra-align inversion region {region_flag}: Query region too short: '
                'Query region length ({:,d}) is not within {:.2f}% of the reference region length ({:,d})'
            ).format(
                region_flag,
    ...
    log_file.write(
        ...
        f'inner={region_ref_inner}, outer={region_ref_outer} (qry inner={region_qry_inner}, outer={region_qry_outer})'
    )
    log_file.flush()
...
#         k_util=None,
#         kde_model=None,
#         max_ref_kmer_count=pavcall_const.INV_MAX_REF_KMER_COUNT,
#         expand_bound=True,
#         force_norm=False,
#         log=None
# ):
...
#
#     if k_util is None:
#         k_util = kanapy.util.kmer.KmerUtil(pavcall_const.INV_K_SIZE)
#
#     if is_rev is None:
#         is_rev = region_qry.is_rev
#
...
#     ref_kmer_count = pavcall_seq.ref_kmers(region_ref_exp, ref_fa_name, k_util)
#
...
#
#     seq_qry = pavcall_seq.region_seq_fasta(region_qry_exp, qry_fa_name, rev_compl=False)
#
#     df = pd.DataFrame(
...
#        pavcall_kde.KMER_ORIENTATION_STATE[
...
#
#
# def resolve_inv_from_rl(df_rl, df, repeat_match_prop=0.2, min_inv_kmer_run=pavcall_const.INV_MIN_INV_KMER_RUN, final_state_node_list=None):
#     """
...
#
#     :return: A tuple of inner and outer regions (`pavcall.region.Region` objects).
#     """
#
#     # Validate walk
...
#     )
#
#     region_qry_inner = pavcall_region.Region(
#         region_qry.chrom,
...
def lift_ref_to_qry(
    ...
    Args:
        region_ref: Reference region to lift.
        align_lift: Alignment lift object.
        index_set: If not `None`, restrict search to this set of aligned indexes.
        match_index: If `True`, then both left and right regions must land on the same alignment record. Set to
            `True` to handle inversions expected to be covered by one contiguous alignment and `False` if the inversion
            might have broken alignments into several aligned fragments.

    Returns:
        A lifted query region or `None` if to lift was found.
    """
    ...
    for lift_l in qry_lift[0]:
        if index_set is not None and lift_l['align_index'] not in index_set:
            continue

        for lift_r in qry_lift[1]:
            if index_set is not None and lift_r['align_index'] not in index_set:
                continue

            # Match reference and orientation
            if lift_l['chrom'] != lift_r['chrom'] or lift_l['is_rev'] != lift_r['is_rev']:
                continue

            # Same alignment record if match_index
            if match_index and lift_l['align_index'] != lift_r['align_index']:
                continue

            # Find best region (best score, break ties by length)
            lift_score = lift_l['score'] + lift_r['score']
            lift_len = np.abs(lift_l['pos'] - lift_r['pos'])
    ...
    return pavcall_region.Region(
        chrom=best_lift[0]['chrom'],
        pos=min(best_lift[0]['pos'], best_lift[1]['pos']),
        end=max(best_lift[0]['pos'], best_lift[1]['pos']),
        is_rev=best_lift[0]['is_rev'],
        align_index_pos=best_lift[0]['align_index'],
        align_index_end=best_lift[1]['align_index']
    )
...
#
#     Args:
#         region_ref: Reference region to expand.
#         df_rl: Run-length encoded table from the inversion search over `region_ref`.
#         df_fai: Reference sequence index.
#     """
#
#     expand_bp = int(len(region_ref) * pavcall_const.INV_EXPAND_FACTOR)
#
...
#
#     :param df_rl: Run-length encoded state table (Generated by pavcall.kde.rl_encoder()).
...
def cluster_merge(
        ...
        pav_params: Optional[pavcall_params.PavParams]
) -> pl.DataFrame:
    """
    Create a table of sites with clustered variants following patterns of intra-alignment inversions.

    Args:
        df_snv: SNV intra-alignment variant table.
        df_insdel: Insertion and deletion intra-alignment variant table.
        df_ref_fai: Reference sequence lengths.
        df_qry_fai: Query sequence lengths.
        pav_params: PAV Parameters.

    Returns:
        Clustered variant table.
    """
    ...
    df_mg = (
        ...
        .agg(
            ...
            pl.col('pos').min(),
            pl.col('end').max(),
            pl.col('flag').explode().implode().unique().sort().alias('flag'),
...
def cluster_insdel(
    ...
    The returned table contains the following columns:
        ...
        count: Number of variants contributing to the cluster.
        depth_max: Maximum depth in the cluster.
        varlen: A list of variant lengths in the cluster.

    Args:
        df: Variant table. Must include INS and DEL variants.
        df_ref_fai: Reference sequence lengths.
        varlen_min: Minimum variant length (Ignore smaller variants).
        offset_prop_max: Maximum offset proportion between INS & DEL variants.
        size_ro_min: Minimum size overlap of INS & DEL variants.

    Returns:
        A table of clustered variants.
    """
    ...
    df_join = agglovar.join.pair.join(
        df_ins, df_del,
        offset_prop_max=offset_prop_max,
        size_ro_min=size_ro_min
    )

    # Get pos and end bounding both INS and DEL for each match
    df_join = (
        df_join
        .join(
            (
                ...
                .select(
                    pl.col('chrom'),
                    pl.col('pos').alias('pos_a'),
                    pl.col('end').alias('end_a'),
                    pl.col('varlen').alias('varlen_a'),
                    pl.col('index_a')
                )
            ),
        ...
        .join(
            (
                df_del
                .with_row_index('index_b')
                .select(
                    pl.col('pos').alias('pos_b'),
                    pl.col('end').alias('end_b'),
                    pl.col('varlen').alias('varlen_b'),
                    pl.col('index_b')
                )
            ),
            on='index_b', how='inner'
        )
        .select(
            pl.col('chrom'),
    ...
    df_depth = pavcall_align.tables.align_depth_table(df=df_join, df_fai=df_ref_fai)

    df_depth = (
        df_depth.lazy()
        .group_by((pl.col('depth') > 0).rle_id())
        .agg(
            pl.col('chrom').first(),
            pl.col('pos').min(),
            pl.col('end').max(),
            (pl.col('index').flatten().unique().len()).alias('count'),
            pl.col('depth').max().alias('depth_max'),
            pl.col('index').flatten().unique(),
        )
        .filter(pl.col('depth_max') > 0)
        # .select(
        #     ['chrom', 'pos', 'end', 'count', 'depth_max']
        # )
    )

    return (
        df_depth
        .with_row_index('depth_index')
        .explode('index')
        .join(
            df_join.select('varlen').with_row_index(),
            on='index',
            how='inner'
        )
        .explode('varlen')
        ...
        .agg(
            ...
            pl.col('end').first(),
            pl.col('count').first(),
            pl.col('depth_max').first(),
            pl.col('varlen').implode()
        )
        .select(
            ['chrom', 'pos', 'end', 'count', 'depth_max', 'varlen']
        )
        .collect()
    )
...
def cluster_sig(
    ...
    Params:
        ...
        df_ref_fai: Reference sequence lengths.
        df_qry_fai: Query sequence lengths.
        cluster_flank: Flank size for clustering. Added upstream and downstream of each variant midpoint before
            clustering.
        varlen_min: Minimum variant length. Ignored if "varlen" is not a column in `df`.
        varlen_max: Maximum variant length. Ignored if "varlen" is not a column in `df`.
        min_depth: Minimum depth for variant clusters.
        min_window_size: Minimum window size for clusters (end - pos).

    Returns:
        Clustered variant regions.
    """
    ...
    df = (
        ...
        .join(
            df_qry_fai.select(
                pl.col('qry_id'), pl.col('len').alias('_qry_len')
            ), on='qry_id', how='left'
        )
        .select(
            pl.col('chrom'),
            ((pl.col('pos') + pl.col('end')) // 2 - cluster_flank).clip(0, pl.col('_qry_len')).alias('pos'),
            ((pl.col('pos') + pl.col('end')) // 2 + cluster_flank).clip(0, pl.col('_qry_len')).alias('end'),
        )
        .filter(pl.col('pos') < pl.col('end'))
    )

    df_depth = pavcall_align.tables.align_depth_table(df=df, df_fai=df_ref_fai, retain_filtered=False)

    return (
        df_depth.lazy()
        .with_columns(
            (pl.col('depth') >= min_depth).rle_id().alias('rle_id'),
        )
        .filter(pl.col('depth') >= min_depth)
        .explode('index')
        .join(
            df.with_row_index('_index'), left_on='index', right_on='_index', how='inner'
        )
        .select(
            pl.col('chrom'),
            pl.col('pos_right').alias('pos'),
            pl.col('end_right').alias('end'),
            pl.col('depth'),
            pl.col('rle_id'),
            pl.col('index')
        )
        ...
        .agg(
            ...
            pl.col('end').max(),
            (pl.col('index').flatten().unique().len()).alias('count'),
            pl.col('depth').min().alias('depth_min'),
            pl.col('depth').max().alias('depth_max'),
        )
        .filter(
            (pl.col('end') - pl.col('pos')) >= min_window_size
        )
        .drop('rle_id', 'depth_min')
    ).collect()
