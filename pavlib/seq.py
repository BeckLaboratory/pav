...
class Region(object):
    ...
    def expand(self, expand_bp, min_pos=0, max_end=None, shift=True, balance=0.5, inplace=True):
        """
        ...
        :param max_end: Upper limit on the new end. This may be an integer defining the maximum end position or a
            Pandas series (key = chrom, value = max end position, e.g. returned from `svpoplib.ref.get_df_fai()` when
            reading a sequence FAI file).
        ...
        :param inplace: If `True`, expand this region in place.

        :return: This region. Allows for chaining or copy before expansion (e.g. `region.copy().expand()`).
        """
        ...
        new_pos = int(new_region.pos - expand_pos)
        new_end = int(new_region.end + expand_end)
        ...
        if max_end is not None:
            if isinstance(max_end, pd.Series):
                ...
                max_end = max_end[new_region.chrom]
            else:
                max_end = None
        ...
        new_region.pos = new_pos
        new_region.end = new_end

        new_region.pos_min = new_region.pos
        new_region.pos_max = new_region.pos
        new_region.end_min = new_region.end
        new_region.end_max = new_region.end
