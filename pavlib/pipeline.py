...
def read_assembly_table(asm_table_filename, config=None, strict_sample_name=False):
    ...
    :param strict_sample_name: If True, sample names must contain only alpha, numeric, dash, and underscore characters.
        If False, sample names may not contain tab characters.
    ...
    if strict_sample_name:
        ...
        bad_name = {name for name in df['NAME'] if pd.isnull(name) or '\t' in name}
